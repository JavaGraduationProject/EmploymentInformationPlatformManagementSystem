...
import com.daowen.entity.Qiye;
import com.daowen.entity.Users;
import com.daowen.service.QiyeService;
...
public class AdminUserController extends SimpleController {
	...
	private void qiyeLogin() {

		String username = request.getParameter("username");
		String password = request.getParameter("password");
		Qiye u = (Qiye) qiyeSrv.load(" where accountname='" + username
				+ "' and password='" + password + "'");
		if (u == null) {
			dispatchParams(request, response);
			request.setAttribute("errmsg",
		...
		HttpSession session = request.getSession();
		session.setAttribute("qiye", u);
		redirect("/qiye/index.jsp");
